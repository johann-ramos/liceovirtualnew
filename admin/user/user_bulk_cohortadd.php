...
foreach ($users as $key => $id) {
    //$user = $DB->get_record('user', array('id'=>$id, 'deleted'=>0), 'id, firstname, lastname, username, email, country, lastaccess, city');
      $user = $DB->get_record('user', array('id'=>$id, 'deleted'=>0), 'id, rut, firstname, middlename, lastname, secondlastname, username, email, country, lastaccess, city');
    $user->fullname = fullname($user, true);
    $user->country = @$countries[$user->country];
    unset($user->firstname);
    unser($user->middlename);
    unset($user->lastname);
    unset($user->secondlastname);
    $users[$key] = $user;
}
unset($countries);
...
$table->width = "95%";
$columns = array('rut', 'username', 'fullname', 'email', 'city', 'lastaccess');
